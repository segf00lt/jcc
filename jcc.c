...
AST_node* declaration(void) {
	...
	e = lexer.text_e;
	unget2 = lexer.unget;
	col2 = lexer.info.col - (int)(lexer.ptr - lexer.unget);
...
/*
 * function: 'func' '(' (vardec (',' vardec)*)? ')' (type)? ('inline'? '{' block '}')
 */
AST_node* function(void) {
	register int t;
	AST_node *root, *child;
	...
	nodes[2] = NULL;
	if(t == T_INLINE) {
		nodes[2] = ast_alloc_node(&ast, N_STORAGEQUALIFIER, keyword[T_INLINE - T_ENUM], &lexer.info);
		t = lex();
	}
	...
	for(; i < ARRLEN(nodes); ++i) {
		if(nodes[i] && nodes[i] != child) {
			child->next = nodes[i];
			child = child->next;
		}
	}
